...
We provide the results reported in the paper in the `processed_results/` folder, and figures can be generated from the results in `results/` using the notebooks in `notebooks/`. Everything will work except "kv_cache.ipynb" and "inference.ipynb" which require the pretrained weights.
